...
| **OpenAI 兼容** | `/v1/chat/completions`（可配 Base URL、API Key、Model）；失败回退站点分组 |
...
| 档位 | 说明 |
| --- | --- |
| **快速** | 单次分类；分批时合并同名/近义组名；会话名用启发式。按站点时此档无影响 |
| **增强** | 主题提示加强 + 域名回填后处理；OpenAI 另做近义组模型合并与模型命名（更慢） |

组名、同站合并与跨站主题剥离在 `extension/lib/groupLabels.js`：twitter.com / x.com 显示为 **X**，且不再从「在 X 上的帖子」切出「上的」。忙的 X / Instagram / TikTok 按作者拆成 `@handle`；剥掉套话后的正文才参与跨站主题（如推文里的 React 与 react.dev）。加站点改这一份即可。
...
- **工具栏弹窗**：收纳、恢复最近会话；立刻整理当前窗口、解散全部窗口标签组、合并打开的重复网页
- **管理页**：全部会话、搜索、预览整理/跨窗合并、去重（打开的 + 已收纳）、闲置、分类设置、导入导出、会话内分组解散
- **快捷键**（可在 `chrome://extensions/shortcuts` 改）：`Alt+Shift+S` 收纳其他标签（保留当前页）；`Alt+Shift+M` 打开标签管理
- **右键菜单**（页面或扩展图标）：收纳当前/全部窗口（默认保留当前页）、立刻整理当前窗口、整理全部窗口并合并到当前（打开管理页确认）

收纳默认**留下当前页**。需要连当前页一起收进会话时，用弹窗或管理页里的「连当前页一起收纳」。右键收纳成功后，工具栏图标会短暂显示数量。

## 自测清单

- [ ] 弹窗：收纳 / 立刻整理当前窗口 / 解散全部窗口标签组 / 合并打开的重复网页 / 恢复最近会话（不跳进管理页）
- [ ] 管理页：预览整理、跨窗合并、打开的+已收纳去重、会话内解散分组
- [ ] 闲置休眠：按 lastAccessed 建议休眠 ≥30 分钟闲置标签（Dev 可附进程内存）
- [ ] 建议分组（站点 / 本地模型 / OpenAI 兼容）
- [ ] 整理当前窗口：跨站主题优先成组；X 忙则按作者拆；同站并入已有组（含单条）；忙 GitHub 按 owner 拆
