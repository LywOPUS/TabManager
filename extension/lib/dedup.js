...
function groupsForDedupKeep(groups) {
  const readLater = [];
  const ungrouped = [];
  const rest = [];
...
/**
 * 打开标签中的重复（默认全部窗口）。
 * @param {{ currentWindowOnly?: boolean }} [opts]
 * @returns {Promise<Array<{ key: string, keep: object, items: object[] }>>}
 */
export async function findOpenTabDuplicates(opts = {}) {
  ...
  for (const [key, list] of byKey) {
    ...
    const items = list.slice(1).map(openTabLoc);
...
function compareOpenTabsForKeep(a, b) {
...
/** 关闭打开标签的重复份，返回关闭数量 */
export async function closeOpenTabDuplicates(dupeGroups) {
  const ids = [];
